...
#     titolo: str = Field(
#         description="Titolo della sezione (può essere un articolo, paragrafo, capitolo, sezione o altro livello identificabile nel testo)"
#     )
#     contenuto: str = Field(
#         description="Testo completo associato a quel titolo"
#     )

# # Definisci il modello Pydantic per l'output completo
# class DocumentoStrutturato(BaseModel):
#     sezioni: List[Sezione] = Field(
#         description="Lista di sezioni del documento, ciascuna con titolo e contenuto"
#     )
    
# output_parser = PydanticOutputParser(pydantic_object=DocumentoStrutturato)
...
# OUTPUT_FILENAME = "pydantic_regolamento_strutturato.json" 
...
def read_document(file_path):
    ...
    if file_path.lower().endswith(".pdf"):
        ...
        raise ValueError("Formato non supportato. Usa .pdf")
...
async def agent_data_extraction(file_path:str, output_json_path:str):
    ...
    try:
        parsed_data = output_parser.parse(response_content)
        # Converte Pydantic model in dict per salvarlo come JSON
        parsed_dict = parsed_data.model_dump()  # converte Pydantic in dict
    except Exception as e:
        print(f"Errore nel parsing con PydanticOutputParser, provo json.loads: {e}")
        try:
    ...
    os.makedirs(os.path.dirname(output_json_path), exist_ok=True)
    with open(output_json_path, "w", encoding="utf-8") as f:
        json.dump(parsed_dict, f, ensure_ascii=False, indent=2)  # parsed_dict
    print(f"File JSON salvato in: {output_json_path}")
